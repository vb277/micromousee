...
# Adjust size for 16x16 maze
maze_size = 16

horizontal_walls = [[0] * maze_size for _ in range(maze_size + 1)]
vertical_walls = [[0] * (maze_size + 1) for _ in range(maze_size)]
...
def direction_cost(current_orientation, current_x, current_y, next_x, next_y, goal_cells):
    # Determine the primary direction to the closest goal cell
    min_manhattan_distance = float('inf')
    goal_x, goal_y = goal_cells[0]

    for gx, gy in goal_cells:
        manhattan_distance = abs(gx - current_x) + abs(gy - current_y)
        if manhattan_distance < min_manhattan_distance:
            min_manhattan_distance = manhattan_distance
            goal_x, goal_y = gx, gy

    # Calculate the direction the robot is moving to
    if next_x > current_x:
    ...
    # Calculate the direction cost based on the turn required
    turn_cost = abs(current_orientation - moving_direction) % 4
    if turn_cost == 3:  # Turning left is sometimes more efficient
        turn_cost = 1
...
def update_walls(x, y, direction, has_wall, horizontal_walls, vertical_walls):
    ...
    if actual_direction == 0:  # NORTH
        if has_wall:
            ...
            if valid_position(x, y + 1, maze_size, maze_size):
                API.setWall(x, y + 1, 's')
        ...
        if has_wall:
            ...
            if valid_position(x + 1, y, maze_size, maze_size):
                API.setWall(x + 1, y, 'w')
...
def can_move(x, y, direction, maze, horizontal_walls, vertical_walls):
    width, height = maze_size, maze_size  # Fixed size for the maze
...
def move_to_lowest_neighbor_with_heuristic(x, y, maze, horizontal_walls, vertical_walls, goal_cells, path=None, phase="initial"):
    ...
    lowest_combined_value = float('inf')
    next_x, next_y = x, y

    log(f"Evaluating neighbors for move from ({x}, {y}): {neighbors}")
    for nx, ny in neighbors:
        base_value = maze[ny][nx]
        dir_cost = direction_cost(current_orientation, x, y, nx, ny, goal_cells)
        combined_value = base_value + dir_cost

        log(f"Neighbor ({nx}, {ny}) has base value {base_value} and direction cost {dir_cost}, combined value {combined_value}")

        if combined_value < lowest_combined_value:
            lowest_combined_value = combined_value
            next_x, next_y = nx, ny

    if lowest_combined_value >= maze[y][x]:
    ...
    log(f"Moving from ({x}, {y}) to ({next_x}, {next_y}) with value {lowest_combined_value}")
