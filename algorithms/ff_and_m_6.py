...
#         f_n = g_n + h_n
#         log(f"Neighbor ({nx}, {ny}): g(n)={g_n}, h(n)={h_n}, f(n)={f_n}")
#         if f_n < lowest_value:
#             lowest_value = f_n
#             next_x, next_y = nx, ny

#     if lowest_value >= maze[y][x] + min(manhattan_distance(x, y, gx, gy) for gx, gy in goal_cells):
