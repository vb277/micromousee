...
def move_to_lowest_neighbour(x, y, maze, horizontal_walls, vertical_walls, goal_cells, path=None, phase="initial"):
    """
    Navigate to the neighbour with the lowest cost (shortest path) and handle wall detection along the way

    Args:
        x (int): Current x-coordinate of the mouse.
        y (int): Current y-coordinate of the mouse.
        maze (list): The maze matrix with distance values.
        horizontal_walls (list): Horizontal walls representation.
        vertical_walls (list): Vertical walls representation.
        goal_cells (list): Coordinates of goal cells.
        path (list): Records the path traversed by the mouse (if provided).
        phase (str): Current phase of the algorithm ('initial', 'return', 'final').

    Returns:
        tuple: Updated (x, y) coordinates after moving to the lowest-cost neighbor.
    """
    ...
    # Evaluate accessible neighbors to find the one with the lowest distance value
    ...
    # Handle the case where we are stuck or no lower-cost neighbors are available
    ...
    # Set appropriate color for the current phase
    if phase == "initial":
    ...
    # Adjust orientation to face the direction of movement
...
def run_flood_fill():
    ...
    while (x, y) not in goal_cells:
        ...
        x, y = move_to_lowest_neighbour(x, y, maze, horizontal_walls, vertical_walls, goal_cells, phase="initial")
        # log(f"Moved to ({x}, {y})")
    ...
    while (x, y) != (0, 0):
        ...
        x, y = move_to_lowest_neighbour(x, y, maze, horizontal_walls, vertical_walls, start_goal, phase="return")
        # log(f"Moved to ({x}, {y})")
    ...
    while (x, y) not in goal_cells:
        ...
        # log(f"Current position: ({x}, {y}), orientation: {current_orientation}")
        x, y = move_to_lowest_neighbour(x, y, maze, horizontal_walls, vertical_walls, goal_cells, path, phase="final")
        # log(f"Moved to ({x}, {y})")
