...
# visit_count = [[0] * 16 for _ in range(16)]
...
# w1_range = np.arange(1, 5, 1)  # Example range for floodfill value weight
# w2_range = np.arange(1, 5, 1)  # Example range for Manhattan distance weight
# w3_range = np.arange(0.5, 2.5, 0.5)  # Example range for exploration preference weight
...
# def scan_and_update_walls(x, y, horizontal_walls, vertical_walls, visit_count):
#     directions = [0, 1, 3]  # NORTH, EAST, WEST
#     visit_count[y][x] += 1  # Increment the visit count
...
#     return neighbors
# def test_weights(w1, w2, w3):
#     # Initialize the micromouse with given weights
#     # Run the simulation
#     # Return the performance metrics (e.g., number of steps)
#     pass
...
#     for w2 in w2_range:
#         for w3 in w3_range:
#             performance = test_weights(w1, w2, w3)
...
#         g_n = maze[ny][nx]
#         h_n = min(manhattan_distance(nx, ny, gx, gy) for gx, gy in goal_cells)
#         revisit_penalty = visit_count[ny][nx] if phase != "final" else 0  # Add a penalty based on the visit count, only if not in final phase
#         f_n = w1 * g_n + w2 * h_n + w3 * revisit_penalty  # Combine the heuristics using weights
#         log(f"Neighbor ({nx}, {ny}): g(n)={g_n}, h(n)={h_n}, penalty={revisit_penalty}, f(n)={f_n}")
#         if f_n < lowest_value:
#             lowest_value = f_n
#             next_x, next_y = nx, ny

#     if lowest_value >= maze[y][x] + min(manhattan_distance(x, y, gx, gy) for gx, gy in goal_cells) + (visit_count[y][x] if phase != "final" else 0):
...
#         neighbors = get_accessible_neighbors(x, y, horizontal_walls, vertical_walls)  # Re-evaluate neighbors after recalculating distances
#         lowest_value = float('inf')
#         for nx, ny in neighbors:
#             g_n = maze[ny][nx]
#             h_n = min(manhattan_distance(nx, ny, gx, gy) for gx, gy in goal_cells)
...
#     scan_and_update_walls(x, y, horizontal_walls, vertical_walls, visit_count)  # Scan walls after moving
...
#         scan_and_update_walls(x, y, horizontal_walls, vertical_walls, visit_count)
